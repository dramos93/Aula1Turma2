...
(
	[idFun]			int				Primary Key			identity(1,1),
...
Insert Into Pedidos(FuncionarioId,ProdutoId,Quantidade) Values(1,3,3)
Insert Into Pedidos(FuncionarioId,ProdutoId,Quantidade) Values(1,4,36)
Insert Into Pedidos(FuncionarioId,ProdutoId,Quantidade) Values(2,1,1)
...
Select Top 1 Pro.NomePro, Sum(ped.Quantidade) as 'Total' From Produtos Pro
Inner Join Pedidos Ped on Ped.ProdutoId = Pro.IdPro
Group By Pro.NomePro
...
Select Top 1 Pro.NomePro, Pro.Valor, Ped.Quantidade, Ped.Quantidade*Pro.Valor as 'Valor Total' From Produtos Pro
Inner Join Pedidos Ped on Ped.ProdutoId = Pro.IdPro
...
Where IdPro Not In
(
Select Ped.ProdutoId From Funcionario Fun
inner Join Pedidos Ped on Fun.idFun = Ped.FuncionarioId
Where Fun.idFun = 1
)
--2.13 - Vamos retornar em uma seleção os produtos ordenados pela ordem do mais comprado para o menos comprado
Select Pro.NomePro as 'Bebida' ,Count(Ped.ProdutoId) as 'Quantidade' From Produtos Pro
left join Pedidos Ped on Pro.IdPro=Ped.IdPed
Group By Pro.NomePro 
